...
$.fn.t=function(content,opts){
 ...
 if(_c=='add'&&$('.t-container',c)[0]){
  ...
  if(!$('.t-container',c)[0]){
  	  c.wrapInner($('<'+_o.tag+'/>',{'class':'t-container'}));
  	  //応急処置
	  c.hide();
	  c.show();
  	  c.css('position').match(/^[ar]/);
	  c.css({
		  'position':'relative',
		  'overflow':'hidden',
		  'font-size':'22px',
		  ...
		  'font-weight':'bold',
		  'text-align':'center',
		  'text-shadow':'2px  2px 3px gray,-2px  2px 3px gray,2px -2px 3px gray,-2px -2px 3px gray',
		  'bottom':'0%',
		  'top':'91%',
		  'color':'white',
		  'user-select':'none',
